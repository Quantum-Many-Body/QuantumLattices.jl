...
Here, `𝕗` (\bbf<tab>) and `𝕓` (\bbb<tab>) are two functions that are convenient to construct and display instances of `FockIndex{:f}` and `FockIndex{:b}`, respectively.
```jldoctest FFF
...
Here, the functions `𝕗` and `𝕓` can also construct and display instances of `Index{<:FockIndex{:f}}` and `Index{<:FockIndex{:b}}`, respectively.
```jldoctest FFF
...
Here, as can be expected, the functions `𝕗` and `𝕓` can construct and display instances of `CoordinatedIndex{<:Index{<:FockIndex{:f}}}` and `CoordinatedIndex{<:Index{<:FockIndex{:b}}}`, respectively, as well.
```jldoctest FFF
...
Here, the type `𝕊` (\bbS<tab>) plays a similar role in spin systems as `𝕗` and `𝕓` in Fock system.
```jldoctest SSS
