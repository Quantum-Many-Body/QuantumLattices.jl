...
Overloaded `==` operator.
"""
Base.:(==)(tc1::TC,tc2::TC) where TC<:TreeCore=all(getfield(tc1,name)==getfield(tc2,name) for name in TC|>fieldnames)
...
macro tree(structdef,treeparams::Union{Expr,Nothing}=nothing)
    ...
    extendbody!(inner,:(new{$(paramnames...)}($(fieldnames...),TreeCore{$(paramnames...)}())))
