...
struct Escaped{N} <: EscapeMechanism
    ...
    Escaped(names::Symbol...) = new{length(names)}(names)
end
...
struct UnEscaped{N} <: EscapeMechanism
    names::NTuple{N, Symbol}
    UnEscaped(names::Symbol...) = new{length(names)}(names)
end
...
function Base.show(io::IO, f::AbstractFactory)
    ...
    for name in fieldnames(typeof(f))
        nstr = " "^2 * string(name) * ":" * " "^(maxfieldnamelength(f)+1-length(string(name)))
        ...
        elseif isa(value, Vector{<:AbstractFactory})
            eltypename = nameof(eltype(value))
            strings = Tuple(string(af(rawexpr)) for af in value)
            if ((length(strings) > 0) ? sum(map(length, strings)) : 0) < 90-maxfieldnamelength(f)
        ...
        ('\n' ∈ vstr) && (vstr = replace(vstr, "\n"=>"\n"*" "^(maxfieldnamelength(f)+4)))
        @printf(io, "%s%s\n", nstr, vstr)
    end
    @printf io ")"
end
...
function Inference(; head::Union{Symbol, Nothing}=nothing, name::Union{Symbol, Nothing}=nothing, params::Union{Inference, Vector{Inference}, Nothing}=nothing)
...
function (p::Parameter)(em::Union{RawExpr, <:UnEscaped, <:MixEscaped})
    (p.name === nothing) ?
        ((p.head == :(::)) ? p.type(em) : Expr(:(<:), p.type(em))) :
        ((p.type === nothing) ? p.name : Expr(:(<:), p.name, p.type(em)))
end
...
Push other parts into the body of a block.
"""
macro push!(b, parts::FExpr...)
    return :(push!($(esc(b)), $parts...))
end
...
Remove line number nodes in the body of a block.
"""
macro rmlines!(b::Expr)
    return :(rmlines!($(esc(b))))
end
...
function (ff::FunctionFactory)(em::Union{RawExpr, <:MixEscaped})
    combinedef(
        Dict(:name          =>      escape(ff.name, isa(em, RawExpr) ? em : em.escaped),
...
Add a couple of positional arguments to a function factory.
"""
macro addargs!(ff, args::FExpr...)
    return :(addargs!($(esc(ff)), $args...))
end
...
Add a couple of keyword arguments to a function factory.
"""
macro addkwargs!(ff, kwargs::FExpr...)
    return :(addkwargs!($(esc(ff)), $kwargs...))
end
...
Extend the body of a function factory.
"""
macro extendbody!(ff, parts::FExpr...)
    return :(extendbody!($(esc(ff)), $parts...))
end
...
"""
macro addwhereparams!(ff, whereparams::FExpr...)
    return :(addwhereparams!($(esc(ff)), $whereparams...))
end
...
function (tf::TypeFactory)(em::Union{RawExpr, <:MixEscaped})
    combinestructdef(
        Dict(:name          =>      escape(tf.name, isa(em, RawExpr) ? em : em.escaped),
            :mutable        =>      tf.mutable,
...
Add a couple of fields to a type factory.
"""
macro addfields!(tf, fields::FExpr...)
    return :(addfields!($(esc(tf)), $fields...))
end
...
Add a couple of constructors to a type factory.
"""
macro addconstructors!(tf, constructors::Expr...)
    return :(addconstructors!($(esc(tf)), $constructors...))
end
...
Add a couple of method parameters to a function factory or a type factory.
"""
macro addparams!(f, params::FExpr...)
    return :(addparams!($(esc(f)), $params...))
end
