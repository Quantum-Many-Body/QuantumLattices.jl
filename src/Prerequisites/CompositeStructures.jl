...
@generated function Base.getindex(ct::CompositeTuple, inds)
    exprs = [(name == :contents) ? :(getfield(ct, :contents)[inds]) : :(getfield(ct, $i)) for (i, name) in enumerate(fieldnames(ct))]
...
@generated function Base.reverse(ct::CompositeTuple)
    exprs = [(name == :contents) ? :(reverse(getfield(ct, :contents))) : :(getfield(ct, $i)) for (i, name) in enumerate(fieldnames(ct))]
    return :(rawtype(typeof(ct))($(exprs...)))
end
...
@generated function Base.getindex(cv::CompositeVector, inds)
    exprs = [(name == :contents) ? :(getfield(cv, :contents)[inds]) : :(getfield(cv, $i)) for (i, name) in enumerate(fieldnames(cv))]
...
@generated function Base.splice!(cv::CompositeVector, range::UnitRange{<:Integer}, replacement=Base._default_splice)
    exprs = [(name == :contents) ? :(splice!(getfield(cv, :contents), range, replacement)) : :(getfield(cv, $i)) for (i, name) in enumerate(fieldnames(cv))]
    return :(rawtype(typeof(cv))($(exprs...)))
end
...
@generated function Base.empty(cv::CompositeVector)
    exprs = [(name == :contents) ? :(empty(getfield(cv, :contents))) : :(getfield(cv, $i)) for (i, name) in enumerate(fieldnames(cv))]
    return :(rawtype(typeof(cv))($(exprs...)))
end
@generated function Base.reverse(cv::CompositeVector)
    exprs = [(name == :contents) ? :(reverse(getfield(cv, :contents))) : :(getfield(cv, $i)) for (i, name) in enumerate(fieldnames(cv))]
    return :(rawtype(typeof(cv))($(exprs...)))
end
@generated function Base.similar(cv::CompositeVector, dtype::Type=eltype(cv), dims::Tuple{Vararg{Int}}=size(cv))
    exprs = [(name == :contents) ? :(similar(getfield(cv, :contents), dtype, dims)) : :(getfield(cv, $i)) for (i, name) in enumerate(fieldnames(cv))]
    return :(rawtype(typeof(cv))($(exprs...)))
end
...
@generated function Base.empty(cd::CompositeDict)
    exprs = [(name == :contents) ? :(Dict{keytype(cd), valtype(cd)}()) : :(getfield(cd, $i)) for (i, name) in enumerate(fieldnames(cd))]
