...
Base.size(vs::VectorSpace) = (dimension(vs),)
...
function _findwithmissing_(basis::B, vs::VectorSpace{B}) where B
    index = searchsortedfirst(vs, basis)
    ((0 < index <= dimension(vs)) && (vs[index] == basis)) ? index : nothing
end
...
struct SimpleVectorSpace{S, B, N} <: VectorSpace{B}
    table::NTuple{N, B}
    function SimpleVectorSpace{S}(table::NTuple{N, B}) where {S, B, N}
        @assert S ∈ ('T', 'F') "SimpleVectorSpace error: wrong type parameter S($S)."
        new{S, B, N}(table)
    end
end
...
struct SimpleIndices{M, N} <: OrderedIndices{N}
    dims::NTuple{N, Int}
    function SimpleIndices{M}(dims::NTuple{N, Int}) where {M, N}
        @assert M ∈ ('C', 'F') "SimpleIndices error: wrong type parameter M($M)."
        new{M, N}(dims)
    end
end
...
"""
Base.keys(nvs::NamedVectorSpace) = keys(typeof(nvs))
