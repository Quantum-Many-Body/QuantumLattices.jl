...
"""
div!(ms::Elements, factor::ScalarElement) = div!(ms, factor.value)
...
!!! note
    ...
    ```
    Here, `M` is the type of the input element `m` in `permute!`, `id1` and `id2` are two arbitary simple ids contained in `m.id`.
"""
