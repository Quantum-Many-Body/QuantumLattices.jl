...
using ..VectorSpaces: GradedTables,GradedVectorSpace,SimpleVectorSpace,TabledIndices
...
"""
    show(io::IO,cid::ID{SimpleID})

Show a composite id.
"""
Base.show(io::IO,cid::ID{SimpleID})=@printf io "ID(%s)" join(cid,",")
...
"""
    IdSpace(sids::SimpleVectorSpace,tables::GradedTables)
    IdSpace(::Type{M},sids::SimpleVectorSpace,gs::Val{GS}) where {M<:AbstractCombinatorics,GS}

The graded id space for an algebra generated by a couple of basic simple ids.
"""
struct IdSpace{S<:SimpleVectorSpace,V<:TabledIndices,T<:GradedTables{Int,V}} <: GradedVectorSpace{Int,Tuple{Vararg{Int}},V,T}
    sids::S
    tables::T
    IdSpace(sids::SimpleVectorSpace,tables::GradedTables)=new{typeof(sids),valtype(tables),typeof(tables)}(sids,tables)
end
IdSpace(::Type{M},sids::SimpleVectorSpace,gs::Val{GS}) where {M<:AbstractCombinatorics,GS}=IdSpace(sids,GradedTables(M,dimension(sids),gs))
...
"""
    findfirst(id::ID{SimpleID},idspace::IdSpace) -> Int
    searchsortedfirst(idspace::IdSpace,id::ID{SimpleID}) -> Int

Find the index of an id in a idspace.
"""
Base.findfirst(id::ID{SimpleID},idspace::IdSpace)=searchsortedfirst(idspace,id)
Base.searchsortedfirst(idspace::IdSpace,id::ID{SimpleID})=searchsortedfirst(idspace,(rank(id),findfirst(id,idspace.sids)))
